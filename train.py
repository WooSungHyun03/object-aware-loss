...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        render_pkg = render(viewpoint_cam, gaussians, pipe, background, need_objectmark=True)
        image, viewspace_point_tensor, visibility_filter, radii = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"]

        gt_image = viewpoint_cam.original_image.cuda()
        gt_mask = viewpoint_cam.gt_alpha_mask.cuda()
        Ll1 = masked_l1_loss(image, gt_image, gt_mask)
        pa_loss = polarization_loss(render_pkg["rend_alpha"], gt_mask)
        po_loss = polarization_loss(render_pkg["rend_object_mark"], gt_mask)
        loss = Ll1 + opt.lambda_pa * pa_loss + opt.lambda_po * po_loss
        ...
        with torch.no_grad():
            ...
            if iteration >= opt.objectmark_prune_from_iter and iteration % opt.objectmark_prune_interval == 0:
                n_pruned = gaussians.prune_background_by_objectmark_score(opt.objectmark_prune_threshold)
                if tb_writer is not None:
