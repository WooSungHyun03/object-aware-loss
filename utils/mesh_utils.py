...
def post_process_mesh(mesh, cluster_to_keep=1000):
    ...
    if cluster_to_keep < 1:
        raise ValueError("cluster_to_keep must be at least 1")
    ...
    if cluster_n_triangles.size == 0:
        return mesh_0
    keep_count = min(cluster_to_keep, cluster_n_triangles.size)
    n_cluster = np.sort(cluster_n_triangles.copy())[-keep_count]
...
class GaussianExtractor:
    def __init__(self, gaussians, render, pipe, bg_color=None):
        ...
        Usage example:
        >>> gaussian_extractor = GaussianExtractor(gaussians, render, pipe)
        >>> gaussian_extractor.reconstruction(view_points)
    ...
    def extract_mesh_bounded(self, voxel_size=0.004, sdf_trunc=0.02, depth_trunc=3, mask_background=True):
        ...
        mask_background: whether to mask background depth when dataset masks exist
        ...
        for i, cam_o3d in tqdm(enumerate(to_cam_open3d(self.viewpoint_stack)), desc="TSDF integration progress"):
            ...
            if mask_background and self.viewpoint_stack[i].gt_alpha_mask is not None:
                gt_alpha_mask = self.viewpoint_stack[i].gt_alpha_mask
                if gt_alpha_mask.device != depth.device:
                    gt_alpha_mask = gt_alpha_mask.to(depth.device, non_blocking=True)
                depth[(gt_alpha_mask < 0.5)] = 0
