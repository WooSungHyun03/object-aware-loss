...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, need_objectmark=False):
    """
    Render the scene. 
    ...
    if need_objectmark:
        # detach geometry so this pass only updates the ObjectMark score, not the Gaussians
        objectmark_opacity = pc.get_objectmark_score_prob
        objectmark_color = torch.ones_like(objectmark_opacity).expand(-1, 3)
        objectmark_settings = raster_settings._replace(bg=torch.zeros_like(bg_color))
        ...
        rets["rend_object_mark"] = (objectmark_image[0:1] + objectmark_aux.sum() * 0.0).clamp(0.0, 1.0)
